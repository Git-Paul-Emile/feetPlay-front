...
const GOOGLE_REDIRECT_PENDING_KEY = 'feetiplay_google_redirect_pending';
...
export function Register() {
  ...
  useEffect(() => {
    if (window.sessionStorage.getItem('feetiplay_google_requires_completion') === '1') {
    ...
    const resolveRedirectResult = async () => {
      ...
      try {
        ...
        if (result.requiresCompletion) {
          navigate('/register/google-complete', { state: { prefill: result.prefill } });
          return;
        }
        navigate('/');
      } catch (err: unknown) {
        setError(firebaseClientErrorToUserMessage(err, 'Inscription Google impossible pour le moment.'));
      } finally {
        if (active) {
          setIsLoading(false);
        }
            ...
            <div>
              ...
              <div className="flex items-center justify-center">
                <button
                  ...
                  className="w-12 h-12 rounded-full bg-white flex items-center justify-center hover:scale-110 transition-transform disabled:opacity-50"
